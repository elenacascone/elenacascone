<div align="center">

# Elena Cascone

**Junior Data Scientist** — Laureata in Fisica | Laureanda Magistrale in Data Science

[![LinkedIn](https://img.shields.io/badge/LinkedIn-1F3864?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/elena-cascone-18ec/)
[![Email](https://img.shields.io/badge/Email-1F3864?style=flat-square&logo=maildotru&logoColor=white)](mailto:elecascone@hotmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-1F3864?style=flat-square&logo=github&logoColor=white)](https://github.com/elenacascone)

</div>

<br>

## Chi sono

Laureata in Fisica (103/110, tesi su entanglement e teletrasporto quantistico), attualmente laureanda magistrale in Data Science (media 29/30). Porto nella data science lo stesso approccio analitico e rigoroso maturato nella formazione fisica: definire ipotesi verificabili, misurare l'incertezza, non fermarsi al primo risultato plausibile.

Attualmente svolgo un tirocinio di ricerca presso il **CNR**, su un progetto di agricoltura di precisione: analisi di dati da sensori IoT (variabili meteo e della pianta) per l'ottimizzazione dell'irrigazione di un vigneto, con sviluppo di un processo di automazione basato su Machine Learning.

**Lingue:** Italiano (madrelingua) · Inglese (B2 Cambridge, 2019) · Francese (base)

<br>

## Progetti

<table>
<tr>
<td width="50%" valign="top">

**[Classificazione di Oggetti Celesti — SDSS](https://github.com/elenacascone/sdss-celestial-classification)**

Confronto tra quattro modelli di classificazione multiclasse (Logistic Regression, KNN, Decision Tree, Random Forest) su dati fotometrici SDSS DR14, con feature engineering via PCA e inferenza statistica tramite bootstrap.

Risultato: Random Forest, 99.1% di accuratezza sul test set.

`Python` · `scikit-learn` · `PCA` · `Bootstrap`

</td>
<td width="50%" valign="top">

**[Previsione della Glicemia — BrisT1D](https://github.com/elenacascone/diabetes-glucose-forecasting)**

Confronto sistematico tra cinque approcci di forecasting (SARIMA, SARIMAX, Prophet, Gradient Boosting, LSTM), con analisi causale via Cross-Correlation Function e interpretabilità tramite SHAP.

Risultato: SARIMA (walk-forward), MAE 0.42 sul test set finale.

`Python` · `statsmodels` · `Prophet` · `LightGBM` · `TensorFlow`

</td>
</tr>
</table>

<br>

## Tech Stack

<div align="center">

![Python](https://skillicons.dev/icons?i=python,tensorflow,sklearn,pandas,numpy,git,github,jupyter,vscode&theme=light&perline=9)

</div>

<br>

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=elenacascone&show_icons=true&hide_border=true&theme=default&title_color=1F3864&icon_color=2E5395&text_color=333333" height="165"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=elenacascone&layout=compact&hide_border=true&theme=default&title_color=1F3864&text_color=333333" height="165"/>

</div>

<br>

<div align="center">

elecascone@hotmail.com · <a href="https://www.linkedin.com/in/elena-cascone-18ec/">LinkedIn</a>

</div>
